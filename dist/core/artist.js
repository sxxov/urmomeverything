...
const paths_js_1 = require("../resources/paths.js");
canvas_1.registerFont(paths_js_1.default.internal.BOLD_FONT_FILE, {
    family: '__font',
    weight: '700',
});
canvas_1.registerFont(paths_js_1.default.internal.REGULAR_FONT_FILE, {
    family: '__font',
    weight: '400',
});
class Artist {
    constructor() {
        this.canvas = canvas_1.createCanvas(Artist.canvasConfig.width, Artist.canvasConfig.height);
        this.canvasCtx = this.canvas.getContext('2d');
    }
    static isPaletteInverted(index) {
        // eg, (4 + 2 == 6) % 6 == 0
        return !!(((((index + 0) % 6) === 0) || (((index + 1) % 6) === 0) || (((index + 2) % 6) === 0)));
    }
    static paintBackground(colour, canvasCtx) {
        canvasCtx.beginPath();
        canvasCtx.rect(0, 0, canvasCtx.canvas.width, canvasCtx.canvas.height);
        canvasCtx.fillStyle = colour;
        canvasCtx.fill();
    }
    static configureText(colour, canvasCtx) {
        canvasCtx.fillStyle = colour;
        canvasCtx.textBaseline = 'middle';
        canvasCtx.textAlign = 'center';
    }
    static measureText(text, font) {
        const canvas = canvas_1.createCanvas(Artist.canvasConfig.width, Artist.canvasConfig.height);
        const canvasCtx = canvas.getContext('2d');
        canvasCtx.font = font;
        return canvasCtx.measureText(text);
    }
    static paintText(text, canvasCtx) {
        const padding = 30;
        const space = 48;
        ...
        switch (true) {
            ...
            case textWidth > workingWidth: {
                ...
                paintRegular(Artist.textConfig.prefix, centre.x, centre.y - Artist.textConfig.fontSize + 30);
                // x: centre, y, centre
                paintBold(`${textLine1}-`, centre.x, centre.y);
                // x: centre, y: below centre
                paintBold(textLine2, centre.x, centre.y + Artist.textConfig.fontSize - 30);
                break;
            }
            // text and prefix too wide to fit inside 'workingWidth'
            case totalWidth > workingWidth: {
                // x: centre, y: above centre
                paintRegular(Artist.textConfig.prefix, centre.x, centre.y - Artist.textConfig.fontSize / 2 + 15);
                // x: centre, y: below centre
                paintBold(text, centre.x, centre.y + Artist.textConfig.fontSize / 2 - 15);
                break;
            }
            // text and prefix can fit inside of 'workingWidth'
            default: {
                // x: depends, y: centre
                paintRegular(Artist.textConfig.prefix, centre.x - (totalWidth / 2) + (prefixWidth / 2) - 8, centre.y);
                // x: depends, y: centre
                paintBold(text, centre.x + (totalWidth / 2) - (textWidth / 2) - 8, centre.y);
            }
        }
        function paintRegular(string, x, y) {
            canvasCtx.font = Artist.fonts.regular;
            canvasCtx.fillText(string, x, y);
        }
        function paintBold(string, x, y) {
    ...
    paint(word, index) {
        ...
        Artist.paintBackground(Artist.isPaletteInverted(index)
            ? Artist.palette.DARK
            : Artist.palette.LIGHT, this.canvasCtx);
        // configure text styles
        Artist.configureText(Artist.isPaletteInverted(index)
            ? Artist.palette.LIGHT
            : Artist.palette.DARK, this.canvasCtx);
        // paint text
        Artist.paintText(word, this.canvasCtx);
        // debugCanvas();
